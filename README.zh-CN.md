...
11. **decompile_by_token** — 仅凭 `MDToken` 反编译方法（或类型），不需要类型名——特别适合直接拿 xref / 字符串搜索结果里的 token（建议带 `assembly_name`,token 是按模块唯一的）。与 `decompile_method` 同样的 async/iterator 兜底
