...
namespace dnSpy.Extension.MCP {
	...
	sealed class McpServer : IDisposable {
		...
		static readonly JsonSerializerOptions jsonOptions = McpDispatcher.JsonOptions;
		...
		public McpServer(McpSettings settings, McpTools tools, BepInExResources bepinexResources) {
			...
			dispatcher = new McpDispatcher(tools, bepinexResources, settings);
		}
		...
		McpResponse HandleRequest(McpRequest request) => dispatcher.Handle(request);
