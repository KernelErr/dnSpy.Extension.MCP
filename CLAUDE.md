...
This is the **dnSpy MCP extension** — a standalone git repo that is cloned *into* a [dnSpyEx](https://github.com/dnSpyEx/dnSpy) checkout at `Extensions/dnSpy.Extension.MCP/`. It is a dnSpy extension (MEF-loaded `*.x.dll`) that runs an HTTP server inside the dnSpy process and exposes 17 assembly-analysis / decompilation / IL-editing / string-search tools and 6 embedded BepInEx docs to AI assistants over Model Context Protocol (JSON-RPC 2.0).
...
- **`McpTools.cs` / `McpTools.IL.cs` / `McpTools.Strings.cs`** — the 17 tools, wrapping `IDocumentTreeView` (loaded-assembly enumeration), `IDecompilerService` (decompilation), and dnlib's `CilBody` (IL view/edit + `ldstr` scan). `McpTools` is a `sealed partial class` split across three files: `McpTools.cs` holds the MEF export, tool schemas, dispatch switch, and shared helpers (`FindAssemblyByName`, `FindTypeInAssembly`, `FindMethod`, cursor codec); `McpTools.IL.cs` holds the IL view/patch/save handlers and the tagged-operand renderer/parser; `McpTools.Strings.cs` holds `search_string_literals` (reverse-lookup a string to the methods that emit it) and `list_string_constants` (all `ldstr` in a type/method), both scanning method bodies via the shared `EnumerateLdstr` helper. List-returning tools use **opaque base64 cursors** (see `EncodeCursor` / `DecodeCursor`) with default page size **10** to stay within AI token budgets. Follow this pattern when adding list tools.
