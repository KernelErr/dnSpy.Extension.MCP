...
This is the **dnSpy MCP extension** — a standalone git repo that is cloned *into* a [dnSpyEx](https://github.com/dnSpyEx/dnSpy) checkout at `Extensions/dnSpy.Extension.MCP/`. It is a dnSpy extension (MEF-loaded `*.x.dll`) that runs an HTTP server inside the dnSpy process and exposes 21 assembly-analysis / decompilation / IL-editing / string-search / cross-reference tools and 6 embedded BepInEx docs to AI assistants over Model Context Protocol (JSON-RPC 2.0).
...
- **`McpTools.cs` / `McpTools.IL.cs` / `McpTools.Strings.cs` / `McpTools.Xref.cs`** — the 21 tools, wrapping `IDocumentTreeView` (loaded-assembly enumeration), `IDecompilerService` (decompilation), and dnlib's `CilBody` (IL view/edit + `ldstr`/operand scan). `McpTools` is a `sealed partial class` split across four files: `McpTools.cs` holds the MEF export, tool schemas, dispatch switch, and shared helpers (`FindAssemblyByName`, `FindTypeInAssembly`, `FindMethod`, cursor codec); `McpTools.IL.cs` holds the IL view/patch/save handlers and the tagged-operand renderer/parser; `McpTools.Strings.cs` holds `search_string_literals` / `list_string_constants` (scanning `ldstr` via the shared `EnumerateLdstr` helper); `McpTools.Xref.cs` holds `find_callers` / `find_references` (sweeping every method body's operands via the shared `ScanForReferences` helper, matching targets by `ResolveMethodDef`/`ResolveFieldDef`/`ResolveTypeDef` with a FullName fallback for refs into unloaded assemblies). Both sweeps run across **all** loaded modules — callers/references routinely live in a different assembly than the target. List-returning tools use **opaque base64 cursors** (see `EncodeCursor` / `DecodeCursor`) with default page size **10** to stay within AI token budgets. Follow this pattern when adding list tools.
