...
This is the **dnSpy MCP extension** — a standalone git repo that is cloned *into* a [dnSpyEx](https://github.com/dnSpyEx/dnSpy) checkout at `Extensions/dnSpy.Extension.MCP/`. It is a dnSpy extension (MEF-loaded `*.x.dll`) that runs an HTTP server inside the dnSpy process and exposes 19 assembly-analysis / decompilation / IL-editing / string-search / cross-reference tools and 6 embedded BepInEx docs to AI assistants over Model Context Protocol (JSON-RPC 2.0).
